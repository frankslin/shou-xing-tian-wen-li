function showHelp(f){
 ...
 　+'　2、日食放大图中，红圆为太阳，黄圆为月亮，它们纵向距离是赤纬差，横向则是赤经差,左东右西。(均不含大气折射修正)。<br>'
   ...
   +'　4、“时间”选择框决定输入与输出所用的时间基准：“所选时区”为页面右上角时区选择框所指地区的区时，“UTC”为世界时，“TD”为力学时。页面每次打开时，右上角时区按浏览器所在时区自动选定，也可自行改选。<br>'
   +'　5、区时由浏览器内置的 IANA 时区库(Intl)查得，夏令时、历史时制变更(如美国二战战时制、我国1986-91年夏令时)均按该库为准，且随浏览器更新。输出的时区标记有当地缩写时用缩写(如 PDT)，否则用 UTC+8 式时差。1900年以前各地通行按经度定的地方平时，时区尚未成立，故一律改用时区表中的标准时差。<br>'
   +'　6、日食发生时，表中可能会出现一个小三角形，它的顶角表示月影轴与地球的交点，在该位置上此刻发生中心食。软件没用帮助用户计算该处发生全食还是环食，用户可以把该处的地标输入，重新计算即可精确判断。<br>'
   +'　7、“NASA”钩上,使用保守日月边缘,结果与NASA网站提供的数据相同<br>'
   +'　8、本图表中，东经为正，西经为负。';
 if(f==3){
  s='';
 }
